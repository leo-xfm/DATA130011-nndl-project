...
model = nn.Model(module = nn.module.CNN_best_simplified(),
                optimizer = nn.optimizer.MomentGD(init_lr=0.01, mu=0.95),
                scheduler = nn.lr_scheduler.MultiStepLR(milestones=[30, 50, 100, 200], gamma=0.5),
                loss_fn = 'CrossEntropyLoss'
                )

model.load_model("../saved_models/best_CNN/best_model.pickle")

save_folder = "../saved_models/best_CNN6666" #!!!!!

# 87.08% 89.71% 90.50% Epoch 285
...
test_images = (load_images('../dataset/t10k-images-idx3-ubyte').reshape(-1, 784) / 255.0).reshape(-1, 1, 28, 28)
test_labels = load_labels('../dataset/t10k-labels-idx1-ubyte')
indices = np.random.permutation(len(train_images))
train_indices = indices[val_size:]
val_indices = indices[:val_size]
train_images, val_images = train_images[train_indices], train_images[val_indices].reshape(-1, 1, 28, 28)
...
for img, label in zip(train_images, train_labels):
    pil_image = Image.fromarray((img * 255).astype(np.uint8).reshape(28, 28))
    augmented_image = nn.transform.transform_image(pil_image)
    augmented_train_images.append(augmented_image.flatten())
    augmented_train_labels.append(label)

augmented_train_images = np.array(augmented_train_images) / 255.0
augmented_train_labels = np.array(augmented_train_labels)
train_images = np.concatenate([train_images, augmented_train_images], axis=0).reshape(-1, 1, 28, 28)
...
for epoch in range(epochs):
    ...
    for it in tqdm(range(int(X.shape[0] / bsz) + 1), desc=f"Epoch {epoch + 1}/{epochs}"):
    ...
    # model.save_model(f"{save_folder}/model_{epoch + 1}.pickle")
